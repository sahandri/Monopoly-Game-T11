			...
			case JAIL:
				...
				if(board.getToken(player).inJail()){
					...
					if(roll_1 == roll_2){ board.getToken(player).release(); }								//release the token from jail state if the 2 numbers thrown are equal
						...
						if((player.getMoney().getMoney()-50 >= 0) && player.getDecision()){		//if the player has enough money to pay for bail and he decides to pay, subtract the money and release him
